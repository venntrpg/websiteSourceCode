<template>
  <div>
    ...
    <button v-on:click="addItemButton()" :disabled="addItemButtonDisabled" class="btn roundedButton widen">Add Item</button>
  </div>
</template>
...
export default {
  ...
  computed: {
    ...
    addItemButtonDisabled () {
      return this.itemName === '' || this.itemBulk === '' || isNaN(parseInt(this.itemBulk)) || this.itemDesc === ''
    },
    consolidatedItems () {
      ...
      for (const item of this.character.items) {
        const index = itemList.findIndex(i => i.name === item.name && i.bulk === item.bulk && i.desc === item.desc)
        if (index === -1) {
          item.count = 1
          itemList.push(item)
        } else {
  ...
  methods: {
    addItemButton () {
      if (!this.addItemButtonDisabled) {
        ...
        this.$store.dispatch('addItem', { id: this.character.id, item: item, refreshCharacter: true })
      }
    }
  }
}
